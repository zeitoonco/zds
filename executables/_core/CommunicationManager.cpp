...
#define lockgc(n, m) unique_lock<mutex> n(mutexes[m])
        ...
        string CommunicationManager::callCommand(string cmdName, string &data, string from, string id,
            ...
            if (cmd != commandList.end()) {
                ...
                cbp.callStartTime = std::chrono::system_clock::now();
            ...
            if (clb != callbackList.end()) {
                //in sync list?
                lDebug("Call Duration:  " + std::to_string(chrono::duration_cast<chrono::milliseconds>( std::chrono::system_clock::now()- clb->second.callStartTime).count()) + " milliseconds. OperationID: " +id);
