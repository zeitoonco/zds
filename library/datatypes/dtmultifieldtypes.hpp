...
public:
	 ...
	 *
	 */
	bool isAllocated(childT *item) const {
		for (citerator i = allocatedOnes.cbegin(); i != allocatedOnes.cend(); i++)
			if (*i == item)
				return true;
		return false;
	}
...
public:
	...
	DTSet(const DTSet &obj) : DTSet(obj.getName()) {
		*this = obj;
	}

	DTSet &operator=(const DTSet &obj) {
		this->setName(obj.getName());
		for (citerator iter = obj.list.begin(); iter != obj.list.end(); iter++) {
			if (obj.isAllocated(*iter)) {
				T *temp = new T("");
				*temp = **iter;
				this->list.push_back(temp);
				this->allocatedOnes.push_back(temp);
			} else
...
public:
	...
	DTStruct &operator=(DTStruct &obj) {
		//TODO: review manually implemented constructors and assignment operators
		//What the hell are these guys doing here?

		/*    this->setName(obj.getName());
			for (citerator iter = obj.list.cbegin(); iter != obj.list.cend(); iter++) {
				DTBase *temp = (DTBase *) (((long int) *iter - (long int) &obj) +
										   ((long int) this));//get addr off field
				this->list.push_back(temp);//push the reference into the list
				temp->fromString((*iter)->toString());
			}*/

	}

	DTStruct(DTStruct &obj) : DTMultiFieldType<DTBase>(obj.getName()) {
		/* this->setVersionValues(obj.version, obj.minVersion, obj.minVersionWOC);
		 *this = obj;*/
	}
	...
	string toString(bool includeVersion, int indent = -1, std::string indentContent = defaultIndentContent,
	                vector<string> *skip = NULL) const {
		string str, ind;
		std::string newLine = "";
		string enc = Strings::indentMaker(indent, indentContent);
		if (indent > 0) {
			newLine = "\n";
			ind = Strings::indentMaker(++indent, indentContent);
		}
		for (citerator i = this->list.begin(); i != this->list.end(); i++) {
			if (skip != NULL && utility::find((*i)->getName(), *skip))
				continue;
			str += ((str.length() > 0) ? "," : "") + newLine + ind + '"' + (*i)->getName() + "\":" +
			       (*i)->toString(indent);
		}
		if (includeVersion)
			str = newLine + ind + "\"_version\":" + to_string(this->getVersion()) + (str.length() > 0 ? "," : "") + str;
		str = enc + newLine + "{" + str + newLine + enc + '}';
		return str;
	}
			...
			if (ready) {
				...
				if (!Strings::compare(name, "_version", false))
					...
					try {
						...
						EXTcantParseStringI(
								"Parsing string '" + value + "' as '" + (*this)[name]->getNameAndType() +
								"' failed",
								excp);
					} catch (exception &excp) {
						EXTcantParseString(
