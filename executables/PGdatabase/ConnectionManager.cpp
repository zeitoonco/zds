...
ConnectionManager::ConnectionManager(std::string adminUserName, std::string adminPassWord, std::string adminHost, int adminPort, std::string adminDbname,
	...
	try {
		...
		if (adminConnection.isConnected()) {
			try {
				...
				EXTDBErrorIO("Admin Database Create Failed", this->getNameAndType(), _errorInfo);
			}
		} else {
			EXTconnectionErrorIO("Admin Connection Failed", this->getNameAndType(), errorInfo);
		}
	}
...
void ConnectionManager::registerNewExtension(std::string extensionName) {
	 ...
	 */
	try {
		adminConnection.execute("CREATE ROLE " + extensionName + " nosuperuser  login password '" + extensionName + "' valid until 'infinity'");
		adminConnection.execute("CREATE SCHEMA " + extensionName);
		adminConnection.execute("GRANT ALL ON SCHEMA " + extensionName + " TO " + extensionName);
		adminConnection.execute(
				"INSERT INTO __local.extension (id,"
						"name, username, password, schema, blocked) values(default, '" + extensionName + "', '" + extensionName + "', '" + extensionName
						+ "', '" + extensionName + "', false)");
		adminConnection.execute("ALTER ROLE " + extensionName + " SET search_path TO " + extensionName); //sets default schema to the newly created one

	} catch (exceptionEx *errorInfo) {
		EXTDBErrorI("unable to register new extension", errorInfo);
	}
	pgMediator->sm.communication.runEvent("database.newUser", extensionName);
	//##Event Fired
}

void ConnectionManager::connectionMaker(std::string extensionName) {
	std::lock_guard<std::mutex> connectionListGuard(mapGuard);
	this->connectionList.insert(std::pair<string, Connection>(extensionName, Connection(extensionName, extensionName, host, port, dbname)));
	pgMediator->sm.communication.runEvent("database.userLogin", extensionName);
	//##Event Fired

}
bool ConnectionManager::checkIfExtensionRegistered(std::string extension) {

	if (adminConnection.query((("select 1 from __local.extension where name =\'" + extension + "\'"))).rowCount() == 1) {
		return true;
	}
	return false;
}
