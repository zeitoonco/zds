...
void ChatCHI::onCommand(string node, string data, string id, string from) {
	if (!Strings::compare(node, CommandInfo::newMessage(), false)) {
		...
		DSMessageList result = chatCore.getMessages(temp.userID.getValue(), temp.sessionID.getValue(), //fixme: @ajl: segfault if we break this line. why? "DSMessageList result;result =  chatCore.getM..."
